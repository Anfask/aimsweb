...
export default function WhyChooseUs() {
    ...
    return (
        <section
            ref={sectionRef}
            className="bg-white pt-6 pb-6 md:pt-12 md:pb-12 font-figtree overflow-hidden border-t border-slate-100"
        >
            <div className="container-custom mx-auto px-6">

                {/* ── Header ── */}
                <div className="wcu-header flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 sm:gap-8 mb-8 sm:mb-12">
                    <div className="space-y-2 sm:space-y-4">
                        <span className="text-[#794d00] font-bold tracking-widest text-[10px] sm:text-xs uppercase flex items-center gap-2">
                            <Sparkles size={12} className="text-blue-500 sm:w-[14px] sm:h-[14px]" /> Educational Framework
                        </span>
                ...
                <div className="features-grid grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
                    {features.map((f, i) => (
                        <div
                            ...
                            className={`feature-card bg-white p-5 pb-3 sm:p-8 rounded-[20px] border border-slate-100 shadow-xl shadow-slate-200/50 transition-all duration-300 flex flex-col gap-3 sm:gap-6 group cursor-pointer ${activeIndex === i ? 'scale-[1.02] border-blue-100 ring-1 ring-blue-50' : 'hover:scale-[1.01]'}`}
                        >
